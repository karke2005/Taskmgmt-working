...
const TaskManagementApp = () => {
  ...
  const [currentView, setCurrentView] = useState(currentUser?.role === "admin" ? "list" : "calendar");
  ...
  const ListView = () => {
    ...
    const categorizeTasksByDueDate = () => {
      ...
      const categories = {
        overdue: { title: "Overdue", tasks: [], color: "bg-red-50 border-red-200", headerColor: "bg-red-100 text-red-800" },
        today: { title: "Due Today", tasks: [], color: "bg-orange-50 border-orange-200", headerColor: "bg-orange-100 text-orange-800" },
        tomorrow: { title: "Due Tomorrow", tasks: [], color: "bg-yellow-50 border-yellow-200", headerColor: "bg-yellow-100 text-yellow-800" },
        thisWeek: { title: "Due This Week", tasks: [], color: "bg-blue-50 border-blue-200", headerColor: "bg-blue-100 text-blue-800" },
        later: { title: "Due Later", tasks: [], color: "bg-gray-50 border-gray-200", headerColor: "bg-gray-100 text-gray-800" },
        noDate: { title: "No Due Date", tasks: [], color: "bg-purple-50 border-purple-200", headerColor: "bg-purple-100 text-purple-800" }
      };
      ...
      return (
        <div className="space-y-8">
          {Object.entries(taskCategories).map(([key, category]) => {
            ...
            return (
              <div key={key} className={`bg-white rounded-xl shadow-sm border-2 ${category.color} overflow-hidden`}>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {category.tasks.map((task) => (
                        <tr key={task.id} className="border-b border-white border-opacity-50 hover:bg-white hover:bg-opacity-30 transition-colors">
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              <div className={`w-1 h-12 rounded-full ${getPriorityColor(task.priority).replace('border-l-', 'bg-')}`}></div>
                              <div>
                          ...
                          <td className="p-4">
                            <span className={`text-xs px-3 py-1 rounded-full font-medium ${getProjectColor(task.project)}`}>
                              {task.project}
                            </span>
                          </td>
                          <td className="p-4">
                            {task.assignedTo ? (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-700 font-medium">{task.assignedTo}</span>
                              </div>
                            ) : (
          ...
          {filteredTasks.length === 0 && (
            <div className="text-center py-16">
              <div className="bg-white rounded-xl shadow-sm border-2 border-gray-200 p-12">
                ...
                <p className="text-gray-600">Create some tasks to get started with your project management</p>
              </div>
            </div>
          )}
        </div>
    ...
    const getDateBadge = (endDate) => {
      if (!endDate) return { text: "No due date", color: "bg-gray-100 text-gray-700" };
      const today = new Date();
      const end = new Date(endDate);
      const diffDays = Math.ceil((end - today) / (1000 * 60 * 60 * 24));

      if (diffDays < 0) return { text: "Overdue", color: "bg-red-100 text-red-700" };
      if (diffDays === 0) return { text: "Due Today", color: "bg-red-100 text-red-700" };
      if (diffDays === 1) return { text: "Due Tomorrow", color: "bg-orange-100 text-orange-700" };
      if (diffDays <= 7) return { text: `Due in ${diffDays} days`, color: "bg-yellow-100 text-yellow-700" };
            ...
            <div className="space-y-4">
              {sortedTasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className={`bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all duration-200 border-l-4 ${getPriorityColor(task.priority)}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${dateBadge.color}`}>
                            {dateBadge.text}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2 py-1 rounded-full ${getProjectColor(task.project)}`}>
                              {task.project}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            ...
                            <div>
                              ...
                              {task.endTime && <span className="ml-1">at {task.endTime}</span>}
                            </div>
                          </div>
  ...
  const GanttView = () => {
    ...
    const groupTasksByProjectAndUser = () => {
      ...
      Object.keys(projectGroups).forEach(project => {
        Object.keys(projectGroups[project]).forEach(user => {
          projectGroups[project][user].sort((a, b) => new Date(a.startDate) - new Date(b.startDate));
        });
      });
    ...
    const allTasks = filteredTasks.sort((a, b) => new Date(a.startDate) - new Date(b.startDate));
    
    const startDates = allTasks.map(task => new Date(task.startDate));
    const endDates = allTasks.map(task => new Date(task.endDate));
    const minDate = startDates.length > 0 ? new Date(Math.min(...startDates)) : new Date();
    const maxDate = endDates.length > 0 ? new Date(Math.max(...endDates)) : new Date();
    ...
    const getTaskPosition = (task) => {
      ...
      const startPos = ((taskStart - bufferedMinDate) / (1000 * 60 * 60 * 24)) * dayWidth;
      const duration = ((taskEnd - taskStart) / (1000 * 60 * 60 * 24) + 1) * dayWidth;
    ...
    const todayLinePosition = todayIndex >= 0 ? 350 + (todayIndex * dayWidth) + (dayWidth / 2) : -1;
      ...
      return (
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-4 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Your task timeline overview</p>
              </div>
          ...
          <div className="overflow-x-auto relative">
            <div className="flex border-b bg-gray-50 sticky top-0 z-10" style={{ minWidth: dateRange.length * dayWidth + 300 }}>
              ...
              {allTasks.map((task, index) => {
                ...
                return (
                  ...
                  >
                    <div className="w-72 p-3 border-r bg-white">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className={`text-xs px-2 py-1 rounded-full inline-block ${getProjectColor(task.project)}`}>
                            {task.project}
                          </div>
                        </div>
                    ...
                    <div className="relative py-3" style={{ width: dateRange.length * dayWidth }}>
                      <div
                        className={`h-6 rounded shadow-md cursor-pointer hover:shadow-lg transition-all duration-200 ${getTaskBgColor(task.priority)} text-white`}
                        style={{ 
                          left: position.left, 
            ...
            {allTasks.length === 0 && (
              <div className="flex items-center justify-center py-12 text-gray-500">
                <div className="text-center">
                  ...
                  <p className="text-sm">Add some tasks to see them in the Gantt chart</p>
                </div>
              </div>
            )}
          </div>
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-4 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Hierarchical project timeline: Project → User → Tasks</p>
            </div>
            <div className="flex items-center gap-4 text-xs">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-red-500 rounded"></div>
        ...
        <div className="overflow-x-auto relative">
          <div className="flex border-b bg-gray-50 sticky top-0 z-10" style={{ minWidth: dateRange.length * dayWidth + 350 }}>
          ...
          <div className="relative">
            {Object.entries(groupedData).map(([projectName, userGroups]) => {
              ...
              const totalTasks = Object.values(userGroups).reduce((sum, tasks) => sum + tasks.length, 0);
              ...
              return (
                <div key={projectName}>
                    ...
                    <div className="w-80 p-4 border-r bg-white">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="font-bold text-gray-900 text-base">{projectName}</div>
                          <div className="text-sm text-gray-600">
                            {Object.keys(userGroups).length} user{Object.keys(userGroups).length !== 1 ? 's' : ''} • {totalTasks} task{totalTasks !== 1 ? 's' : ''}
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="relative flex-1" style={{ width: dateRange.length * dayWidth }}>
                      {/* Project timeline bar showing overall span */}
                      {(() => {
                        const projectTasks = Object.values(userGroups).flat();
                        if (projectTasks.length > 0) {
                          const projectStart = new Date(Math.min(...projectTasks.map(task => new Date(task.startDate))));
                          const projectEnd = new Date(Math.max(...projectTasks.map(task => new Date(task.endDate))));
                          const startPos = ((projectStart - bufferedMinDate) / (1000 * 60 * 60 * 24)) * dayWidth;
                          const duration = ((projectEnd - projectStart) / (1000 * 60 * 60 * 24) + 1) * dayWidth;
                          
                          return (
                            <div 
                              className="absolute top-1/2 transform -translate-y-1/2 h-3 bg-purple-300 rounded opacity-60"
                              style={{ left: startPos, width: Math.max(duration, dayWidth) }}
                            />
                          );
                        }
                        return null;
                  ...
                  {isExpanded && Object.entries(userGroups).map(([userName, userTasks]) => (
                      ...
                      >
                        <div className="w-80 p-3 border-r bg-white">
                          <div className="flex items-center gap-3 ml-8">
                            <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-sm font-bold">
                              {userName === 'Unassigned' ? '?' : userName.split(' ').map(n => n[0]).join('')}
                            </div>
                            <div className="flex-1">
                              <div className="font-semibold text-gray-900">{userName}</div>
                              <div className="text-sm text-gray-600">{userTasks.length} task{userTasks.length !== 1 ? 's' : ''}</div>
                            </div>
                          </div>
                        </div>
                        <div className="relative flex-1" style={{ width: dateRange.length * dayWidth }}>
                          {/* User timeline bar */}
                          {(() => {
                            const userStart = new Date(Math.min(...userTasks.map(task => new Date(task.startDate))));
                            const userEnd = new Date(Math.max(...userTasks.map(task => new Date(task.endDate))));
                            const startPos = ((userStart - bufferedMinDate) / (1000 * 60 * 60 * 24)) * dayWidth;
                            const duration = ((userEnd - userStart) / (1000 * 60 * 60 * 24) + 1) * dayWidth;
                          ...
                          >
                            <div className="w-80 p-3 border-r bg-white">
                              <div className="flex items-start gap-2 ml-16">
                                <div className={`w-1 h-8 rounded mt-1 ${getPriorityColor(task.priority).replace('border-l-', 'bg-')}`}></div>
                                <div className="flex-1 min-w-0">
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {currentUser.role === "user" && currentView === "calendar" && <CalendarView />}
